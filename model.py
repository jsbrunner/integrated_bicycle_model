...
def micromodel(seed = 4,  # random seed
               ...
               demand = [30, 45],  # list with demands []
               path_width = 4,  # width of the simulated path (m, incl. 2x 0.5 m space on side of the path)
               v0_mean = 5,  # mean of desired speed (m/s)
               v0_sd = 2,  # standard deviation of desired speed (m/s)
               ...
               check_cyclist_id = 10,  # follow the choices of an individual cyclist with his unique_id; put nan for no output
               b_length = 2,  # bicycle length (m)
               b_width = 0.8,  # bicycle width (m)
               d_standing = 0.1,  # minimum standing distance to other cyclists (m)
               a_des = 1,  # relaxation time for acceleration (s)
               b_max = 2,  # maximum braking (m/s^2, >0)
               omega_max = 0.5,  # maximum lateral speed (m/s)
               omega_des = 0.2,  # desired lateral speed (m/s)
               phi = 5,  # scale length of the consideration range (-)
               alpha = 1,  # scale length of the safety region (-)
    ...
    **********************
    ...
    ''' make sure beforehand that the inflow step is a multiple of the time step length; create a user warning if this does not fit '''
    
    # We assume that bicycles are generated with a same interval (uniformly distributed) according to the demand.
    random.seed(seed)  # set the seed
    time_steps = int(duration/dt)
    interval = [int(time_steps*0.5 / demand[i]) for i in range(len(demand))] # Time interval in both of the two slots
    print(interval)
    inflow_step = [] # time points that bicycles enter the bike lane
    for i in range(len(demand)):
        inflow_step.extend(list(range(0 + int(time_steps/2) * i, int(time_steps/2) * (i+1), interval[i])))
    print(inflow_step)
    ...
    class Bicycle(Agent):
        ...
        ************************************
        ...
        def __init__(self, unique_id, model):
            ...
            ''' maybe shift to triangular distribution '''
            self.v0 = random.uniform(v0_mean - v0_sd, v0_mean + v0_sd)  # distribution of desired speed
            ...
            self.sr_length = self.v0 * self.alpha  # length of the safety region
            self.sr_width = self.v0 * self.beta  # width of the safety region
            self.cr_length = self.v0 * self.phi  # consideration range length
    ...
    ***********************
    ...
    ''' change data frame like currently in the run file '''
