...
class Player(FuncAnimation):  # Player class from https://stackoverflow.com/questions/44985966/managing-dynamic-plotting-in-matplotlib-animation-module/44989063#44989063
    def __init__(self, fig, func, frames=None, init_func=None, fargs=None,
                 save_count=False, mini=0, maxi=300, pos=(0.04, 0.02), **kwargs):
        self.i = 0
        self.min=mini
        self.max=maxi
        self.runs = True
...
def plot_simulation(agent_pos, 
                    dt = 0.2, 
                    anim_interval = 200, # time to update (ms); 200 ms = 5 FPS
                    plot_length = [0,100],  # start and end of space to show the simulation (m)
                    check_cyclist_id = 10,
    ...
    class Player(FuncAnimation):  # Player class from https://stackoverflow.com/questions/44985966/managing-dynamic-plotting-in-matplotlib-animation-module/44989063#44989063
        def __init__(self, fig, func, frames=None, init_func=None, fargs=None,
                     save_count=False, mini=0, maxi=len(agent_pos['Step'].unique()), pos=(0.04, 0.02), **kwargs):
            self.i = 0
            self.min=mini
            self.max=maxi
            self.runs = True
    ...
    def animate(frame):
        ...
        # naming the plot
        ax.set_title(f'Time {minutes:02}:{seconds:02}.{hundredth:02}  |  Step {frame:04}  |  dt = {dt}  |  {datetime.now().strftime("%Y-%m-%d_%H:%M")} ')
    ...
    anim = Player(fig, animate, frame=agent_pos['Step'].unique(), interval=anim_interval, cache_frame_data=True)
